...
public class IMAPProtocol extends Protocol {
     ...
     * @see "RFC2087"
     */
    public void setQuota(Quota qr) throws ProtocolException {
	if (!hasCapability("QUOTA")) 
	    throw new BadCommandException("QUOTA not supported");

	Argument args = new Argument();	
	args.writeString(qr.quotaRoot);
	Argument qargs = new Argument();	
	if (qr.resources != null) {
	    for (int i = 0; i < qr.resources.length; i++) {
		qargs.writeAtom(qr.resources[i].name);
		qargs.writeNumber(qr.resources[i].limit);
	    }
	}
